...
clinvar_file_name<-'clinvar_plp_20240708'
...
# Full table
full_clinvar_coverage_file_name<-glue('clinvar_coverage.{target_file_name}.{clinvar_file_name}.all_variants.tsv')
write.table(clinvar_coverage_table%>%select(-synonyms),file=glue('./output/clinvar_coverage/{full_clinvar_coverage_file_name}'),
            sep='\t',row.names = F,quote = F)
# Missing table
missing_clinvar_coverage_file_name<-glue('clinvar_coverage.{target_file_name}.{clinvar_file_name}.missing_variants.tsv')
...
per_gene_name<-glue('clinvar_coverage.{target_file_name}.{clinvar_file_name}.per_gene')
