...
giab_strats_folder<-glue('./data/giab_stratification_files/')
giab_strats_files<-list.files(giab_strats_folder,recursive = T,pattern = '\\.bed',full.names = T)

# genes files
gene_files<-list('refseq'='./data/accessory_files/GCF_000001405.25_GRCh37.p13.refseq.bed')
...
for (giab_strat_file in giab_strats_files){
  ...
  intersection_command<-glue('bedtools coverage -a {gene_files[[gene_file_name]]} -b {giab_strat_file}')
...
write.table(gene_giab_intersection,file=glue('./output/gene_giab_stratifications/gene_giab_stratifications.csv'),row.names = F,quote = F,sep = '\t')
...
write.table(gene_giab_intersection_filtered,
            file=glue('./output/gene_giab_stratifications/gene_giab_stratifications.disclaimer.csv'),
